...
export class TransactionService {
  public static async getFistPage(): Promise<TransactionModel[]> {
    ...
    for (const item of data) {
      const block = await BlockModel.findOne({where: {number: item.blockNumber},
  ...
  public static async getTransAllByBlockNumber(blockNumber): Promise<TransactionModel[]> {
    const data = await TransactionModel.findAll({where: {blockNumber}, raw: true})
    return data.map((item) => {
      return this.transFormatter(item)
    })
  }

  public static async getTransByBlockNumber(blockNumber: number, pageSize = 10, pageNumber = 1):
  Promise<{transactions: any[]; count: number}> {
    ...
    const transactions = await Bluebird.all(result.map(async (item) => {
  ...
  public static async getTransByHash(hash): Promise<TransactionModel> {
    ...
    if (result.length === 0) {
      throw createHttpError({status: 200, message: 'this hash not found', type: 'transaction', code: 50001})
    }
    const transaction: TransactionModel = result[0]
    const [block, currentBlockNumber] = await Bluebird.all([
      BlockModel.findOne({where: {number: transaction.blockNumber},
        attributes: ['timestamp'],
        raw: true}),
    ...
    if (block) {
      ...
      transaction['confirmations'] = currentBlockNumber - transaction.blockNumber
    }
  ...
  public static async getByHashFormatter(hash): Promise<TransactionModel> {
    ...
    const block = await BlockModel.findOne({where: {number: trans.blockNumber},
      attributes: ['timestamp'],
      raw: true})
    if (block) {
  ...
  public static async getTransByFromOrTo(account: string, pageNumber = 1, pageSize = 10):
  ...
  public static async getTransCountByFromOrTo(account: string): Promise<number> {
  ...
  public static async getTransByHashOrTo(hash: string, address: string, pageNumber = 1, pageSize = 10):
  ...
  public static async getTransCountByHashOrTo(hash: string, address: string): Promise<number> {
    const counts = await dbPool.query(`
        select count(1) count from t_transactions 
        where hash = ${Buffer.from(hash).toString('binary')}
        or \`to\` =  ${Buffer.from(address).toString('binary')}
  ...
  public static async getTotalTransFee() {
    const result = await dbPool.query('select sum(txFee) total from t_blocks_chart', {
      replacements: [],
      type: 'SELECT',
    })
